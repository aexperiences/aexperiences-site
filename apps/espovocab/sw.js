/* ESPOvocab service worker — network-first for the page, stale-while-revalidate for assets.
...
self.addEventListener("activate", function (e) {
  e.waitUntil(caches.keys().then(function (ks) {
    return Promise.all(ks.filter(function (k) { return k.indexOf("espovocab-") === 0 && k !== C; }).map(function (k) { return caches.delete(k); }));
